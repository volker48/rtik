...
**update_ticket — dynamic SQL with dynamically built parameter list:**

rusqlite returns an error if a named parameter is present in the bind list but absent from the
SQL query. Because the SET clause is built dynamically (only fields that are Some), the parameter
list must also be built dynamically to match exactly. Use a Vec of (&str, &dyn ToSql) pairs and
call execute_named with only the params that appear in the SQL:
...
pub fn update_ticket(
    ...
    if let Some(s) = status {
        normalized_status = s.to_lowercase();
        sets.push("status = :status");
        params.push((":status", &normalized_status));
    }
...
Note: `params.as_slice()` works because rusqlite's `execute` accepts `&[(&str, &dyn ToSql)]`
via the `Params` trait implementation for slices of named pairs. Only the params that appear
in the SQL are included, so rusqlite never sees an unmatched named parameter.

Also note: status normalization (`.to_lowercase()`) happens here in update_ticket, not in the
CLI layer, so the normalized value is what gets stored regardless of call site.
...
Add `use rusqlite::Connection;` at the top of the file.
  ...
  <verify>
    ...
    update_ticket builds params Vec dynamically — only params matching the SET clause are included.
    Verify partial update: `update_ticket(&conn, id, None, None, Some("WIP"))` compiles and runs without
    rusqlite error about unmatched named parameters.
  </verify>
  <done>
    ...
    update_ticket builds named params dynamically — only params in the SET clause are bound.
    update_ticket validates NoUpdateFields before building SQL.
    Status is normalized to lowercase inside update_ticket.
    `cargo check` passes.
  </done>
</task>
