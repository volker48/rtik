...
<task type="auto">
  ...
  <action>
Create `tests/integration.rs` with a full integration test suite. Each test uses an isolated
in-memory (or tempfile) database to avoid test order dependencies.
...
fn open_test_db() -> rusqlite::Connection {
    ...
    db::open_connection(tmp.path()).expect("open_connection failed")
    // Note: tmp is dropped here but the file path is still valid for the connection
    // Use keep() or store tmp alongside conn in a struct if WAL tests need the file to persist
}
```

Wait — tempfile drops (deletes) the file when tmp goes out of scope. For WAL mode tests,
use a persistent temp path instead:
...
```
Expected: compilation failure due to missing `tempfile` dev-dependency.
