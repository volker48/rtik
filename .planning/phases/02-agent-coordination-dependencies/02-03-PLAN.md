...
must_haves:
  truths:
    ...
    - "rtik deps #ID shows dependency tree for a ticket"
  ...
  artifacts:
    ...
    - path: "src/cli.rs"
      provides: "Dep subcommand with add/remove/show sub-subcommands"
      contains: "Dep"
    - path: "src/lib.rs"
      provides: "Dispatch for Dep::Add, Dep::Remove, Dep::Show"
      contains: "Commands::Dep"
  key_links:
...
<task type="auto">
  ...
  <action>
**In src/cli.rs — add Dep subcommand:**
...
RTIK_DB=/tmp/deps.db cargo run -- dep show 2    # Depends on: #1 / Required by: #3
...
```
  ...
  <done>
    ...
    - `dep show` outputs correct forward and reverse dep lists
